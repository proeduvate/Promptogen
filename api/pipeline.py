...
async def start_pipeline(request: PipelineStartRequest) -> PipelineStartResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Clarification failed: {exc}") from exc

    questions = result.get("questions", [])
    _state_store[thread_id] = {"original_query": request.query, "questions": questions}
...
@router.post("/continue", response_model=PipelineContinueResponse, status_code=status.HTTP_200_OK,
...
async def continue_pipeline(request: PipelineContinueRequest) -> PipelineContinueResponse:
    stored = _state_store.pop(request.thread_id, None)
    if stored is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"thread_id '{request.thread_id}' not found. Call /pipeline/start first.")
    logger.info("POST /pipeline/continue | thread_id=%s answers=%d", request.thread_id, len(request.answers))
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Generation/assessment failed: {exc}") from exc

    logger.info("Pipeline complete | thread_id=%s overall_score=%s", request.thread_id, result.get("overall_score"))
